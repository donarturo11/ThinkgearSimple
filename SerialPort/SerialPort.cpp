...
void SerialPort::enumerateWin32Ports(){
...
*/
//----------------------------------------------------------------
void SerialPort::buildDeviceList(){
	deviceType = "serial";
	devices.clear();
...
//----------------------------------------------------------------
vector <SerialDeviceInfo> SerialPort::getDeviceList(){
	buildDeviceList();
	return devices;
}

//----------------------------------------------------------------
void SerialPort::enumerateDevices(){
	listDevices();
}
...
//----------------------------------------------------------------
bool SerialPort::setup(int deviceNumber, int baud){
	buildDeviceList();
...
//----------------------------------------------------------------
bool SerialPort::setup(string portName, int baud){
	bInited = false;
		...
		break;
		   default:
			cfsetispeed(&options, B9600);
		cfsetospeed(&options, B9600);
		std::cout << "setup(): cannot set " << baud << " bps, setting to 9600" << std::endl;
		break;
		}
...
//----------------------------------------------------------------
long SerialPort::writeBytes(const unsigned char * buffer, size_t length){
	return writeBytes(reinterpret_cast<const char*>(buffer), length);
}

//----------------------------------------------------------------
bool SerialPort::writeByte(char singleByte){
...
//----------------------------------------------------------------
long SerialPort::readBytes(unsigned char * buffer, size_t length){
...
bool SerialPort::writeByte(unsigned char singleByte){
	if (!bInited){
		std::cerr << "writeByte(): serial not inited\n";
		return false;
	}
